...
data.forEach(version => {
  test.describe(version + ': Concatenate', () => {
    test('Concatenating 2 and a results in 2a', async ({ page }) => {
      await page.goto('https://testsheepnz.github.io/BasicCalculator');
      await page.selectOption('#selectBuild', { label: version});
      await page.locator('#number1Field').type('2');
      ...
      await expect(page.locator('#numberAnswerField')).toHaveValue('2a');
    });
  });
});
...
data.forEach(version => {
  test.describe(version + ': Concatenate', () => {
    test('Concatenating 0.6 and 0.2 results in 0.60.2', async ({ page }) => {
      await page.goto('https://testsheepnz.github.io/BasicCalculator');
      await page.selectOption('#selectBuild', { label: version});
      ...
      await expect(page.locator('#numberAnswerField')).toHaveValue('0.60.2');
    });
  });
});
